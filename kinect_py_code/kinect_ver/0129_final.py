...
################ kife and bat #########################
#model_name = 'training_kb_batch8_eff0_40000_fintuned_model'
#model_name = 'training_kb_batch12_eff0_40000_fintuned_model'
model_name = 'training_kb_batch12_eff0_50000_fintuned_model'
    ...
    while True:
        ...
        if final_box is not None:
            for i in range(len(final_box)):
                ...
                cv2.circle(image_obj, (x,y), 10, (255, 0, 0), -1)
                ## draw for just check!!
                #cv2.line(image_obj, (int(final_box[i][0]), int(final_box[i][3])),
                #         (int(final_box[i][1]), int(final_box[i][3])), (255, 0, 0), 5, 4)

                #cv2.line(image_obj, (int(final_box[i][0]), int(final_box[i][2])),
        ...
        if Kinect._bodies is not None:
            ...
            if diff is not None:
                ...
                img_np = cv2.resize(image_np, (640, 480))
                d = img_np.flatten()
                strr = d.tostring()
