...
host_temp = ['192.168.0.42','192.168.0.42']
global cnt
cnt = True

#### for video s
fourcc = cv2.VideoWriter_fourcc(*'DIVX')
out = cv2.VideoWriter('saved.mp4', fourcc, 25.0, (640, 480))
t_end = time.time() + 10
        ...
        if Kinect._bodies is not None:
            if Kinect._kinect.has_new_depth_frame:
                for i in range(0, Kinect._kinect.max_body_count):
                    ...
                    #right hand
                    r_x = int(depth_points[PyKinectV2.JointType_HandRight].x)
                    r_y = int(depth_points[PyKinectV2.JointType_HandRight].y)

                    r_depth = Kinect._kinect.get_last_depth_frame()
                    r_z = int(r_depth[r_y * 512 + r_x])

                    array_rx.append(r_x)
                    array_ry.append(r_y)
                    array_rz.append(r_z)  # array의 필요성..?

                    #left hand
                    l_x = int(depth_points[PyKinectV2.JointType_HandLeft].x)
                    l_y = int(depth_points[PyKinectV2.JointType_HandLeft].y)

                    l_depth = Kinect._kinect.get_last_depth_frame()
                    l_z = int(l_depth[l_y * 512 + l_x])

                    array_lx.append(l_x)
                    array_ly.append(l_y)
                    array_lz.append(l_z)  # array의 필요성..?

                    print("right_hand_depth : ", r_z)
                    print("left_hand_depth : ", l_z)
